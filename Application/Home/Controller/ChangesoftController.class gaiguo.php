        ...
        if ($softid != '') {
            ...
            if ($ret[0]['authmode'] == 0) {
                ...
                if ($ret[0]['authmode'] == 1) {
                    ...
                    if ($ret[0]['authmode'] == 2) {
                        $this->assign('singleloginurl', $ret[0]['loginurl']);
                        $this->assign('ggurl', $ret[0]['ggurl']);
                        $this->assign('dqtimeurl', $ret[0]['dqtimeurl']);
                        $this->assign('yiyouver', $ret[0]['yiyouver']);
                        $this->assign('color', $ret[0]['color']);
                    } else {
                        if ($ret[0]['authmode'] == 3) {
                            $this->assign('web' . $ret[0]['web'], 'checked');
            ...
            if ($user_id[0]['vip'] == 0) {
                if ($a != '') {
                    if (count($a) == 1) {
                        HJCTool::alertToLocation('免费用户限制只能创建一个应用', 'managesoft');
                    }
                }
            }
    ...
    private function change()
    {
        ...
        if ($auth == 0) {
            ...
            if (I('sid') == '') {
                ...
                $sql = $mysql->fetchSql(true)->add($data);
                echo $sql;
                if(1==1){
                // if (!$updateret) {
            ...
            if ($auth == 1) {
                ...
                if ($auth == 2) {
                    //易游单码模式
                    $title = I('title');
                    $loginurl = I('singleloginurl');
                    $color = I('color');
                    $ggurl = I('ggurl');
                    $dqtimeurl = I('dqtimeurl');
                    $yiyouver = I('yiyouver');
                    $frozen = I('frozen');
                    $create_time = date('Y-m-d H:i:s', time());
                    if (I('sid') == '') {
                        // 新建
                        $updateret = $mysql->execute("INSERT INTO cloud_software (authmode,create_time, user_id, name, title, loginurl,color,ggurl,dqtimeurl,yiyouver,frozen) VALUES (2,'{$create_time}', '{$user_id}', '{$software}', '{$title}', '{$loginurl}', '{$color}', '{$ggurl}', '{$dqtimeurl}', '{$yiyouver}','{$frozen}')");
                        if (!$updateret) {
                            HJCTool::alertBack('新建失败');
                        } else {
                            HJCTool::alertToLocation('新建成功', 'managesoft');
                        ...
                        // 修改
                        $sid = I('sid');
                        $updateret = $mysql->execute("UPDATE cloud_software SET user_id = '{$user_id}', name = '{$software}', title = '{$title}', loginurl = '{$loginurl}', color = '{$color}',ggurl = '{$ggurl}',dqtimeurl = '{$dqtimeurl}', yiyouver = '{$yiyouver}',frozen ='{$frozen}' WHERE id = '{$sid}'");
                        if (!$updateret) {
                            HJCTool::alertBack('修改失败');
                        } else {
                            // 这里是为了修改软件名的同时修改注册码表的软件名,但是如果软件名没有改,这句就会返回失败,所以不用管
                    ...
                    if ($auth == 3) {
                        ...
                        if (I('sid') == '') {
                            // 新建
                            $updateret = $mysql->execute("INSERT INTO cloud_software (authmode,create_time, user_id, name, try_minutes, try_count, update_url, frozen,updatamsg,title,sy,gg,web,weburl,hint,edit) VALUES (3,'{$create_time}', '{$user_id}', '{$software}', '{$try_minutes}', '{$try_count}', '{$update_url}', '{$frozen}','{$updatamsg}','{$title}','{$sy}','{$gg}','{$web}','{$weburl}','{$hint}','{$edit}')");
                            if (!$updateret) {
                                HJCTool::alertBack('新建失败');
                            } else {
                                HJCTool::alertToLocation('新建成功', 'managesoft');
                            ...
                            // 修改
                            $sid = I('sid');
                            $updateret = $mysql->execute("UPDATE cloud_software SET user_id = '{$user_id}', name = '{$software}', try_minutes = '{$try_minutes}', try_count = '{$try_count}', update_url = '{$update_url}',frozen ='{$frozen}',title ='{$title}',sy ='{$sy}',gg = '{$gg}',web= '{$web}',weburl='{$weburl}',hint='{$hint}',edit='{$edit}' WHERE id = '{$sid}'");
                            if (!$updateret) {
                                HJCTool::alertBack('修改失败');
                            } else {
                                // 这里是为了修改软件名的同时修改注册码表的软件名,但是如果软件名没有改,这句就会返回失败,所以不用管
                        ...
                        if ($auth == 4) {
                            ...
                            if (I('sid') == '') {
                                // 新建
                                $updateret = $mysql->execute("INSERT INTO cloud_software (authmode,create_time, user_id, name, try_minutes, try_count, update_url, frozen,updatamsg,title,sy,gg,web,weburl,hint,edit) VALUES (4,'{$create_time}', '{$user_id}', '{$software}', '{$try_minutes}', '{$try_count}', '{$update_url}', '{$frozen}','{$updatamsg}','{$title}','{$sy}','{$gg}','{$web}','{$weburl}','{$hint}','{$edit}')");
                                if (!$updateret) {
                                    HJCTool::alertBack('新建失败');
                                } else {
                                    HJCTool::alertToLocation('新建成功', 'managesoft');
