...
class CreatecodeController extends BaseController {
    ...
    private function init() {
        ...
        //HJCTool::alertBack(json_encode($this->_softArray));
        if ($this->_softArray == '') {
    ...
    private function produceCode() {
        ...
        $unUse = $ret[0]['count(*)'];
        $max = 5;
         if($this->getUser()[0]['vip'] == 0){
              if ($unUse + I('count') > $max) {
                   HJCTool::alertBack('免费用户未使用注册码张数不能超过' . $max . '张,您最多还可以生成' . ($max - $unUse) . '张');
               }
         } else if($this->getUser()[0]['vip'] == 1){
          $max=5000;
          if ($unUse + I('count') > $max) {
