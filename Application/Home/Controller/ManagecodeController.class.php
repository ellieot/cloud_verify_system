...
class ManagecodeController extends BaseController {
    ...
    private function exportTxt() {
        $today = date('Ymd');
        $filename='卡密_' . $today . '.txt';//要导出的文件的文件名需要加上文件后缀
        ...
        if ($this->finalSql) {
            ...
            $userId = $this->getUserId();
            $ret = $mysql->query("SELECT code FROM cloud_regcode WHERE user_id = '$userId'");
        }
        $chars = '';
        foreach ($ret as $json) {
        ...
        header('Cache-Control: must-revalidate, post-check=0, pre-check=0');    //不让浏览器缓存，不然每次都会是一样的
        //突然想到验证码的时候，浏览器会缓存验证码，造成网页垃圾，如果用这个会不会好一点呢
        //所以列入TODO吧，反正是期末作业也别指望有谁会看
