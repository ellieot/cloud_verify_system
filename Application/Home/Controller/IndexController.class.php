...
class IndexController extends BaseController {
    ...
    private function queryCode() {
        ...
        $overdue_ret = $mysql->query("SELECT COUNT(*) FROM cloud_regcode WHERE user_id = '$userId' AND overdue = 1" . $sqlTail);        
        $online_ret = $mysql->query("SELECT COUNT(*) FROM cloud_regcode WHERE user_id = '$userId' AND isonline = 1" . $sqlTail);
        $this->assign('unuse_count', $unuse_ret[0]['count(*)']);//未使用
        $this->assign('use_count', $use_ret[0]['count(*)']);//使用中
        $this->assign('online_count', $online_ret[0]['count(*)']);//在线
        $this->assign('overdue_count', $overdue_ret[0]['count(*)']);//过期
    }
