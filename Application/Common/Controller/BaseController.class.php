...
class BaseController extends Controller {
    ...
    private $_itemCountAPage = 15;  //每页显示多少个
    private $_pageCount;
    private $_userId = -1;
    ...
    public function __construct() {
        ...
        // 接口url不用判断登录
        if (strstr($_SERVER[REQUEST_URI], '/Home/Verify') || strstr($_SERVER[REQUEST_URI], '/home/test')) {
            return;
        }
        ...
        } else if($_SESSION['user'] != 'admin123'){
            // 超过多少分钟未操作,清除登录状态，自己就不用跳了
...
protected function getUser(){
    ...
    protected function showTableAndPage($table_name, $sql) {
        ...
        if ($sql) {
            ...
            $limitSql = " WHERE user_id = '$userId'";
            $ret = $mysql->query("SELECT * FROM cloud_" . $table_name . $limitSql);
        }
        $this->_pageCount = ceil(sizeof($ret) / $this->_itemCountAPage);    //ceil()向上取整

        
        if ($this->isEmpty(I('get.page', '', '/^[1-9]\d*$/'))) {
            $this->_currentPage = 1;    //如果他乱输get参数就跳转到第一页去
        } else {
            // 超过最大页数
        ...
        if ($sql) {
            ...
            $onePageSql = "SELECT * FROM cloud_" . $table_name . $limitSql . " LIMIT " . (($this->_currentPage - 1) * $this->_itemCountAPage) . ", " . $this->_itemCountAPage;
        }
...
protected function showTableAndPage2($table_name, $sql) {
        // 先查询总数,计算出页数
        $mysql = M(ucfirst($table_name)); // 首字母大写
        $ret = $mysql->query("SELECT * FROM cloud_user");
